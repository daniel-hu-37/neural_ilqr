...
class iLQR(BaseController):
    ...
    def __init__(self, dynamics, cost, N, max_reg=1e10, hessians=False):
        ...
        self._k = np.zeros((N, dynamics.dim_control))
        self._K = np.zeros((N, dynamics.dim_control, dynamics.dim_state))
    ...
    def _control(self, xs, us, k, K, alpha=1.0):
        ...
        disturbances = np.array([[np.random.choice([-0.1, 0], p=[0.3, 0.7])] for i in range(len(us))])
        # disturbances = np.zeros_like(us)
    ...
    def _forward_rollout(self, x0, us):
        ...
        """
        dim_state = self.dynamics.dim_state
        dim_control = self.dynamics.dim_control
        N = us.shape[0]

        xs = np.empty((N + 1, dim_state))
        F_x = np.empty((N, dim_state, dim_state))
        F_u = np.empty((N, dim_state, dim_control))

        L = np.empty(N + 1)
        L_x = np.empty((N + 1, dim_state))
        L_u = np.empty((N, dim_control))
        L_xx = np.empty((N + 1, dim_state, dim_state))
        L_ux = np.empty((N, dim_control, dim_state))
        L_uu = np.empty((N, dim_control, dim_control))
    ...
    def _backward_pass(self, F_x, F_u, L_x, L_u, L_xx, L_ux, L_uu):
        ...
        for i in range(self.N - 1, -1, -1):
            Q_x, Q_u, Q_xx, Q_ux, Q_uu = self._Q(
                F_x[i], F_u[i], L_x[i], L_u[i], L_xx[i], L_ux[i], L_uu[i], V_x, V_xx
            )
    ...
    def _Q(self, f_x, f_u, l_x, l_u, l_xx, l_ux, l_uu, V_x, V_xx):
        ...
        # Eqs (11b) and (11c).
        reg = self._mu * np.eye(self.dynamics.dim_state)
...
class RecedingHorizonController(object):
    ...
    def control(
        ...
        **kwargs
        ...
        """
        dim_control = self._controller.dynamics.dim_control
        ...
        while True:
            ...
            us_end = self._random.uniform(-1, 1, (step_size, dim_control))
